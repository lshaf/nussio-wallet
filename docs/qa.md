...
| Microsoft Edge | `pnpm test:e2e:edge`                                                         | onboarding suite passed                                   |
| Brave          | `pnpm test:e2e:brave`                                                        | onboarding suite passed                                   |
| Firefox MV2    | `pnpm build:firefox && pnpm exec web-ext lint --source-dir dist/firefox-mv2` | 0 errors, 3 warnings                                      |
| Bundle budgets | `pnpm check:bundle`                                                          | popup 518 KB, prompt 576 KB, app 1106 KB, 2.43 MB total   |
...
Firefox has no automated coverage because MV2 loads the extension through `web-ext`, not
Playwright. Walk this by hand with `pnpm dev:firefox`:

1. Onboarding: password → enable Jungle 4 → import a watch account → home lists the balance.
2. Link capture: open a page with an `esr:` link, click it, prompt window opens (Firefox falls back
   to the injected `inpage.js` because MV2 ignores `world: 'MAIN'`).
