...
| Access to all sites | Blockchain API nodes, history nodes and the signing relay are addresses the user configures, so no fixed host list can cover them. The content script reads no page content; it only recognises `esr:` signing-request links, which can appear on any site. |
